...
#BEGIN INPUT
#gmx_plumed topology for protein
inpTOPprot=../00-prep/topol_prot.top
#gmx_plumed files from MD sampling
...
#gmx_plumed group number for protein (gmx_plumed trjconv)
outGrp=1
#gmx_plumed itp file with nonbonded parameters
gmx_plumedDATA=$HOME/gromacs-2022.5-plumed-2.8.2
ffnonbonded=$gmx_plumedDATA/share/top/amber99sb-ildn.ff/ffnonbonded.itp
...
gmx_plumed trjconv -s ${inpTPR} -f ${inpTRR} -o ${outTRRprotAA} -pbc mol << STOP >& trjconv1.out
${outGrp}
STOP

#Generate all-atom reference structure for rot+trans fitting
gmx_plumed trjconv -s ${inpTPR} -f ${outTRRprotAA} -o ref.gro -e 0.0 << STOP >& trjconv2.out
${outGrp}
STOP
#some postprocessing for PLUMED compatibility of ref.pdb
gmx_plumed editconf -f ref.gro -o ref.pdb
...
gmx_plumed trjconv -s tmptraj.gro -f tmptraj.gro -timestep ${TRJtimestep} -o ${outTRRprotCG} << STOP >& trjconv3.out
0
STOP
