...
gmx_plumed grompp -f sample-NPT.mdp -c ${inpGRO} -p ${inpTOP} -o sample-NPT.tpr >& grompp.out
export OMP_NUM_THREADS=16
gmx_plumed mdrun -v -deffnm sample-NPT -cpi -ntomp 16 -pin on -pme gpu -gpu_id 0 >& mdrun.out
