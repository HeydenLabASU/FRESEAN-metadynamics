...
void printReferenceFile(t_atom *atoms, int nAtoms, int *nRes){
	for(int j=0;j<nAtoms;j++) {	
		if(j == nAtoms - 1){
			*nRes = atoms[j].resNr + 1;
		}
	}
}
...
void printCustomTopology(FILE *topol, int nAtoms, t_atom *atoms, int nRes, int *nBeads){
		...
		for(int j=0;j<nAtoms;j++){	
			...
			*/
			if(isBackboneOnly(atoms[j].resName)){
				assignCOM(COM_POS_BB, COM_VEL_BB, &BB_MASS, atoms[j]);
			}
			...
			*/
				...
				if(inBackbone(atoms[j])){
					assignCOM(COM_POS_BB, COM_VEL_BB, &BB_MASS, atoms[j]);
				}
				else{
					assignCOM(COM_POS_SC, COM_VEL_SC, &SC_MASS, atoms[j]);
				}
			}
			...
			*/
			if(j == nAtoms - 1 || atoms[j+1].resNr != curr_index){
				...
				NAMES[curr_index] = atoms[j].resName;
...
int main(int argc, char *argv[])
{
	...
	*/
    ...
    for(i=0;i<nRead;i++){
		...
		if(i == 0){
			printReferenceFile(atoms, nAtoms, &nRes);
			printCustomTopology(topol, nAtoms, atoms, nRes, &nBeads);
		...
		*/
		if(isBackboneOnly(atoms[j].resName)){
			assignCOM(COM_POS_BACKBONE, COM_VEL_BACKBONE, &BACKBONE_MASS, atoms[j]);
		}
		...
		*/
			...
			if(inBackbone(atoms[j])){
				...
				assignCOM(COM_POS_SIDECHAIN, COM_VEL_SIDECHAIN, &SIDECHAIN_MASS, atoms[j]);
			}
		}
		...
		*/
		...
		if(j == nAtoms - 1 || atoms[j+1].resNr != curr_index){
			if(isBackboneOnly(atoms[j].resName) == 0){
				
				divideCOM(COM_POS_BACKBONE, COM_VEL_BACKBONE, BACKBONE_MASS);
				divideCOM(COM_POS_SIDECHAIN, COM_VEL_SIDECHAIN, SIDECHAIN_MASS);
				printBackbone(out, atoms[j], COM_POS_BACKBONE, COM_VEL_BACKBONE, &beadCounter);
				printSidechain(out, atoms[j], COM_POS_SIDECHAIN, COM_VEL_SIDECHAIN, &beadCounter);
